...
predict.npcs <- function(object, newx, ...) {
  ...
  } else if (object$classifier == "logistic-l1-contrast") {
    D <- data.frame(x = newx, y = 1)
    x.model.new <- model.matrix(y~.-1, D)
    posterior.test <- as.matrix(predict_fit(object$fit$beta.min, as.matrix(x.model.new), type = "prob"))
  } else if (object$classifier == "xgboost") {
    D <- data.frame(x = newx, y = 1)
    x.model.new <- model.matrix(y~.-1, D)
    posterior.test <- predict(object$fit, x.model.new, reshape = TRUE)
  }
